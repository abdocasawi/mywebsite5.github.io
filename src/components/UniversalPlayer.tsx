...
type PlayerType = 'auto' | 'ksplayer' | 'vlc' | 'enhanced' | 'hls' | 'native';
...
export const UniversalPlayer: React.FC<UniversalPlayerProps> = ({ channel, onChannelEnd }) => {
    ...
    return (
      <div className="aspect-video bg-gray-900 rounded-xl flex items-center justify-center">
        <div className="text-center text-gray-400">
          ...
          <p className="text-sm mt-2">Universal IPTV Player with KSPlayer</p>
        </div>
      </div>
    );
  }

  // Auto-detect player type based on URL - KSPlayer as primary choice
  const getAutoPlayerType = (): PlayerType => {
    const url = channel.url.toLowerCase();
    
    // For all IPTV streams, prefer KSPlayer for professional streaming
    if (url.includes('.m3u8') || url.includes('hls') || url.includes('iptv') || url.includes('.mpd')) {
      return 'ksplayer';
    }
    
    // For direct video files, still use KSPlayer for consistency
    if (url.match(/\.(mp4|webm|ogg|avi|mkv|mov|flv)(\?|$)/)) {
      return 'ksplayer';
    }
    
    // Default to KSPlayer for all formats
    return 'ksplayer';
  };
  ...
  const playerOptions = [
    { type: 'auto' as PlayerType, label: 'Auto Detect', icon: Settings, description: 'Automatically select KSPlayer for all streams (Recommended)' },
    { type: 'ksplayer' as PlayerType, label: 'KSPlayer', icon: Star, description: 'Advanced IPTV player for all formats (Primary)' },
    ...
    <div className="relative">
            ...
            <div className="absolute right-0 top-12 bg-gray-900 border border-gray-700 rounded-lg shadow-xl min-w-80 overflow-hidden">
              ...
              <div className="py-2">
                {playerOptions.map(({ type, label, icon: Icon, description }) => (
                  ...
                  >
                    <Icon className={`w-5 h-5 mt-0.5 flex-shrink-0 ${type === 'ksplayer' ? 'text-purple-400' : ''}`} />
                    <div className="text-left">
                      <div className="font-medium flex items-center space-x-2">
                        <span>{label}</span>
                        {type === 'ksplayer' && <span className="text-xs bg-purple-500 text-white px-1 rounded">PRO</span>}
                      </div>
              ...
              <div className="p-3 border-t border-gray-700 bg-gray-800">
                <div className="text-xs text-gray-400 space-y-1">
                  <p><strong>KSPlayer:</strong> Advanced streaming engine for all IPTV formats (Default)</p>
      ...
      <div className="absolute bottom-4 left-4 bg-black bg-opacity-75 rounded-lg px-3 py-2 text-white text-sm">
        <div className="flex items-center space-x-2">
          ...
          {getCurrentPlayerType() === 'ksplayer' && (
            <span className="text-xs bg-purple-500 text-white px-1 rounded">PRO</span>
          )}
        </div>
      </div>
    </div>
